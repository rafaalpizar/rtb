...
get_bible() {
	sed '1,/^#EOF$/d' < "$SELF" | tar xzf - -O bible/"$1"
}
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_bible kjv.tsv | awk -v cmd=list "$(get_data process.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_bible kjv.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data process.awk)" | ${PAGER}
	done
	exit 0
fi

get_bible kjv.tsv | awk -v cmd=ref -v ref="$*" "$(get_data process.awk)" | ${PAGER}

BEGIN {
    ...
    if (len_to < len_from)
        for (; i <= len_from; i++)
            t_ar[substr(tr_from, i, 1)] = substr(tr_to, len_to, 1)
    # ends transliterate init
...
function stranslate(target,     ltarget, c, result) {
    # Transform (transliterate) special characters to improve search
    # also lower case the string.
    #
    # target: the string to remove special characters
    # ltarget = length from target string
...
cmd == "ref" && mode == "search" && (p["book"] == "" || bookmatches($1, $2, p["book"])) && (p["chapter"] == "" || $4 == p["chapter"]) && match(stranslate($6), stranslate(p["search"])) {
    processline()
}
